...
class BottomupGetHeatmapMask(BaseTransform):
    ...
    def _segs_to_mask(self, segs: list, img_shape: Tuple[int,
        ...
        # details can be found at `COCO tools <httpxxx'
        ...
        if results.get('bbox', None) is not None:
            ...
            if results.get('keypoints', None) is not None:
                ...
                results['keypoints_visible'] = results['keypoints_visible'][
                    valid_inds]
            #print(f'/data/mmpose/datasets/transforms/bottomup_transforms.py\n {results}')
            if results.get('segmentation') is not None and isinstance(results['segmentation'], np.ndarray):
                # 
